...
class ApiService {
  ...
  static Future<CourseModel?> fetchCourseById(int id) async {
    return _fetchData<CourseModel>('$baseUrl/courses/$id', (json) => CourseModel.fromJson(json));
  }

  static Future<Handbook?> fetchHandbookById(int id) async {
    return _fetchData<Handbook>('$baseUrl/handbooks/$id', (json) => Handbook.fromJson(json));
  }

  static Future<T?> _fetchData<T>(String url, T Function(Map<String, dynamic>) fromJson) async {
    try {
  ...
  static Future<void> handleNotificationNavigation(String type, int itemId) async {
    if (type == 'news') {
